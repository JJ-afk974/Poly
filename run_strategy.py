...
def get_best_prices(book):
    ...
    # Les carnets Polymarket peuvent être renvoyés
    # dans différents ordres : on prend explicitement
    # le maximum du bid et le minimum de l'ask.
...
def fetch_snapshot():
    ...
    for market_key, slug in slugs.items():
        ...
        for market in markets:
            ...
            market_active = market.get(
                "active",
                event_active
            )

            market_closed = market.get(
                "closed",
                event_closed
            )

            market_resolved = market.get(
                "resolved",
                event_resolved
            )
...
def resolve_positions(
    ...
    for token_id, position in portfolio["positions"].items():
        ...
        # Dans cette version, on utilise les données
        # présentes dans le snapshot.
        #
        # Si le token est résolu mais qu'on ne connaît
        # pas explicitement le résultat, on évite de
        # solder arbitrairement la position.
        ...
        # Si aucune information de résolution explicite
        # n'est disponible, on ne touche pas à la position.
...
def calculate_portfolio_value(
    ...
    for token_id, position in portfolio["positions"].items():
        ...
        if mid is not None:
            ...
            # Si le marché n'est plus disponible,
            # on ne valorise pas artificiellement.
            #
            # La position sera conservée jusqu'à
            # résolution.
...
def strategy(
    ...
    """
    ========================================================
    STRATEGIE TEMPORAIRE
    ========================================================

    Cette fonction est volontairement séparée du moteur.

    Tu peux remplacer son contenu par ta propre stratégie.
    ...
    for _, row in snapshot.iterrows():
        ...
        if ask is not None:
            ...
            current_position_value = (
                existing_quantity * ask
            )


            if (
                ask <= BUY_THRESHOLD
                and current_position_value < MAX_POSITION_USD
            ):
